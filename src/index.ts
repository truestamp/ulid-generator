...
export { MonotonicULID } from './montonicULID'
...
async function handleRequest(request: Request, env: Env) {
  ...
  const id = env.MONOTONIC_ULID.idFromName('default') // 'default' is an arbitrary name, and will always return the same id
  const stub = env.MONOTONIC_ULID.get(id)
  const response = await stub.fetch('http://object/?q=' + quantity) // the hostname 'object' is arbitrary, and will always point to the Durable Object
